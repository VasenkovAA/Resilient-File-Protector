...
CryptoPanel::CryptoPanel(Role role, QWidget *parent)
  ...
  setTitle(role == Role::Embed ? tr("Encryption") : tr("Decryption"));
  setCheckable(true);
  setChecked(false);
  ...
  passwordEdit_->setPlaceholderText(role == Role::Embed
                                        ? tr("Enter password")
                                        : tr("Enter password to decrypt"));
  ...
  if (role_ == Role::Embed) {
    ...
    for (const auto &spec : rfp::crypto::CryptoRegistry::availableKdfs()) {
      if (spec.id == rfp::crypto::KdfId::Argon2id) {
        continue;
      }
  ...
  hintLabel_->setText(
      role_ == Role::Embed
          ? tr("Password is never written to disk or settings. "
               "Cipher, KDF and iterations are stored inside the encrypted "
               "payload.")
          : tr("Cipher, KDF and iterations are read from the payload — "
  ...
  connect(revealButton_, &QToolButton::toggled, this,
          &CryptoPanel::onToggleReveal);

  if (confirmEdit_)
    connect(confirmEdit_, &QLineEdit::textChanged, this, &CryptoPanel::changed);
  if (cipherCombo_)
    connect(cipherCombo_, &QComboBox::currentIndexChanged, this,
            &CryptoPanel::changed);
  if (kdfCombo_)
    connect(kdfCombo_, &QComboBox::currentIndexChanged, this,
            &CryptoPanel::changed);
  if (iterationsSpin_)
    connect(iterationsSpin_, &QSpinBox::valueChanged, this,
            &CryptoPanel::changed);
...
rfp::crypto::CipherId CryptoPanel::cipher() const {
  if (!cipherCombo_)
    return rfp::crypto::CipherId::Aes256Gcm;
  return static_cast<rfp::crypto::CipherId>(
      cipherCombo_->currentData().toInt());
}

rfp::crypto::KdfId CryptoPanel::kdf() const {
  if (!kdfCombo_)
    return rfp::crypto::KdfId::Pbkdf2HmacSha256;
...
std::optional<QString> CryptoPanel::validatedPassword() const {
  ...
  if (passwordEdit_->text().isEmpty())
    return std::nullopt;
  if (role_ == Role::Embed && passwordEdit_->text() != confirmEdit_->text())
    return std::nullopt;
...
void CryptoPanel::setIterations(std::uint32_t n) {
  if (iterationsSpin_)
    iterationsSpin_->setValue(static_cast<int>(n));
}

void CryptoPanel::onToggleEnabled(bool on) {
  passwordEdit_->setEnabled(on);
  revealButton_->setEnabled(on);
  if (confirmEdit_)
    confirmEdit_->setEnabled(on);
  if (confirmLabel_)
    confirmLabel_->setEnabled(on);
  if (cipherCombo_)
    cipherCombo_->setEnabled(on);
  if (kdfCombo_)
    kdfCombo_->setEnabled(on);
  if (iterationsSpin_)
    iterationsSpin_->setEnabled(on);
  hintLabel_->setEnabled(on);
  emit changed();
}
